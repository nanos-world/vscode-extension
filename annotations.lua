...
CollisionChannel = {
    WorldStatic = 1 << 0, -- WorldStatic Object Types
    ...
    All = (1 << 32) - 1, -- All Object
}

---@enum CollisionType
CollisionType = {
    Normal = 0, -- Blocks All
    StaticOnly = 1, -- Only Blocks Static objects
    NoCollision = 2, -- Doesn't Block anything
    IgnoreOnlyPawn = 3, -- Blocks everything but Pawns (Characters)
    Auto = 4, -- Automatically selects - usually will be Normal. On Props it will switch between <b>Normal</b> and <b>IgnoreOnlyPawn</b> depending on the Prop siz
}
...
---@enum SkyMode
SkyMode = {
    VolumetricClouds = 0, -- Uses a 3D Volumetric Cloud layer
...
---@enum SteeringType
SteeringType = {
    SingleAngle = 0, -- Both wheels steer by the same amount
    AngleRatio = 1, -- Outer wheels on corner steer less than the inner ones by set ratio
    Ackermann = 2, -- Ackermann steering principle is applie
}
...
SuspensionSweepShape = {
    ...
    Spherecast = 1, -- Use sphere to determine suspension length to ground
    Shapecast = 2, -- Use wheel collision shape to determine suspension length to ground - Slowes
}
...
TraceMode = {
    ...
    TraceOnlyVisibility = 1 << 6, -- Whether should only trace for objects that don't ignore visibility collision channel
...
ViewMode = {
    ...
    TPS3 = 3, -- Third Person Far View
    TopDown = 4, -- Top Down Vie
}
...
---@enum WidgetSpace
WidgetSpace = {
    World = 0, -- The widget is rendered in the world as mesh, it can be occluded like any other mesh in the world
    Screen = 1, -- The widget is rendered in the screen, completely outside of the world, never occlude
}